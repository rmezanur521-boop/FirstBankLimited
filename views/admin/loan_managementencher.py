...
class LoanManagement(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        for label, val in filters:
            btn = ctk.CTkButton(
                ...
                font=("Arial", 12, "bold"),
                corner_radius=6,
                fg_color=BLUE if val == "pending" else "#EEF2F7",
                text_color=WHITE if val == "pending" else DARK,
                hover_color="#1558B0" if val == "pending" else "#DDE3EC",
        ...
        self.table_frame.pack(fill="both", expand=True, padx=20, pady=12)
    ...
    def _load(self, status=None):
        ...
        # Update summary
        total_amount = sum(float(ln["loan_amount"]) for ln in loans)
        self.summary_label.configure(
            text=f"  Showing {len(loans)} loan(s)  |  "
        ...
        if not loans:
            ...
            ctk.CTkLabel(
                empty,
                text="📭  No loan applications found for this filter.",
    ...
    def _render_table(self, loans):
        headers = [
            ...
            "Amount", "Rate", "Tenure",
            "EMI", "Outstanding", "Status", "Applied On", "Actions"
        ]
        widths = [140, 150, 130, 110, 60, 70, 110, 120, 90, 140, 180]
        ...
        for i, ln in enumerate(loans):
            ...
            # Status badge
            ctk.CTkLabel(
                row, text=ln["status"].upper(),
                font=("Arial", 10, "bold"), text_color=WHITE,
                fg_color=sc, corner_radius=6,
                width=widths[8], anchor="center"
            ).pack(side="left", padx=5, pady=7)

            # Applied on
            ctk.CTkLabel(
                row, text=format_datetime(ln["applied_at"]),
                font=("Arial", 11), text_color=DARK,
    ...
    def _approve(self, loan_id):
        ...
        if not loan:
            show_message(self, "Loan not found.", "error")
            return

        msg = (
            f"Approve this loan application?\n\n"
            f"  Customer  : {loan['customer_name']}\n"
            f"  Loan Ref  : {loan['loan_ref']}\n"
            ...
            f"The loan amount will be credited to the customer's account immediately."
        )
        ...
        try:
            ...
            show_message(
                self,
                f"✅ Loan approved!\n\n"
                f"{format_currency(loan['loan_amount'])} has been credited "
                f"to account {loan['account_number']}.",
                "success"
            )
            self._load(self._current_filter)
        except Exception as e:
            show_message(self, f"Error approving loan: {e}", "error")
...
class _RejectDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, loan, user, on_done=None):
        ...
        self.title("Reject Loan Application")
    ...
    def _build(self):
        ...
        ).pack(padx=28, fill="x", pady=(4, 8))

        ctk.CTkButton(
            self, text="Cancel",
            fg_color="#7A8BA0", hover_color="#546E7A",
            font=("Arial", 12),
